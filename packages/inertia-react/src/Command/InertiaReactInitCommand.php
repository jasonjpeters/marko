...
namespace Marko\Inertia\React\Command;
...
#[Command(name: 'inertia-react:init', description: 'Ensure the minimum package.json setup required for Inertia React')]
readonly class InertiaReactInitCommand implements CommandInterface
{
    public function __construct(
        private PackageJsonUpdater $packageJson,
        private InertiaReactPublisherInterface $publisher,
        private InertiaReactViteConfigUpdater $viteConfigUpdater,
    ) {}
    ...
    public function execute(
        ...
        $configResult = $this->viteConfigUpdater->ensureReactConfig($force, $dryRun);
        $entrypointResult = $this->publisher->publishJsEntrypoint($force, $dryRun);

        $this->report($result, $dryRun, $output);
        $this->reportConfig($configResult, 'React-aware Vite config', $output);
        $this->reportConfig($entrypointResult, 'Inertia React entrypoint', $output);
