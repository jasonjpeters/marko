...
use Marko\Inertia\Svelte\Command\InertiaSvelteInitCommand;
...
beforeEach(function (): void {
    $this->tempDirectory = sys_get_temp_dir() . '/marko-inertia-svelte-init-command-' . bin2hex(random_bytes(6));
...
function makeInertiaSvelteInitCommand(string $directory): InertiaSvelteInitCommand
{
    ...
    return new InertiaSvelteInitCommand(
        new PackageJsonUpdater($paths),
        new InertiaSveltePublisher($viteConfig, $publisher),
        new InertiaSvelteViteConfigUpdater($viteConfig, $paths, $publisher),
    );
}

function captureInertiaSvelteInitOutput(InertiaSvelteInitCommand $command, array $argv): array
{
...
test('inertia svelte init dry run reports planned changes without writing files', function (): void {
    [$status, $output] = captureInertiaSvelteInitOutput(
        makeInertiaSvelteInitCommand($this->tempDirectory),
        ['marko', 'inertia-svelte:init', '--dry-run'],
    );

    expect($status)->toBe(0)
        ->and($output)->toContain('Would create package.json')
        ->and($output)->toContain('Added devDependency `svelte`')
        ->and($output)->toContain('Added devDependency `@sveltejs/vite-plugin-svelte`')
        ->and($output)->toContain('Added devDependency `@inertiajs/svelte`')
        ->and($output)->toContain('Would publish vite.config.ts (Svelte-aware Vite config)')
        ->and($output)->toContain('Would publish resources/js/app.ts (Inertia Svelte entrypoint)');
...
test('inertia svelte init force replaces existing bootstrap files and updates package json', function (): void {
    ...
    [$status, $output] = captureInertiaSvelteInitOutput(
        makeInertiaSvelteInitCommand($this->tempDirectory),
        ['marko', 'inertia-svelte:init', '--force'],
    );
    ...
    expect($status)->toBe(0)
        ...
        ->and($output)->toContain('Updated devDependency `svelte`')
        ->and($output)->toContain('Updated vite.config.ts (Svelte-aware Vite config)')
        ->and($output)->toContain('Updated resources/js/app.ts (Inertia Svelte entrypoint)')
        ...
        ->and($packageJson['devDependencies']['svelte'])->toBe('latest')
        ->and($packageJson['devDependencies']['@sveltejs/vite-plugin-svelte'])->toBe('latest')
        ->and($packageJson['devDependencies']['@inertiajs/svelte'])->toBe('latest')
