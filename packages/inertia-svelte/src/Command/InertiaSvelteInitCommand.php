...
namespace Marko\Inertia\Svelte\Command;
...
#[Command(name: 'inertia-svelte:init', description: 'Ensure the minimum package.json setup required for Inertia Svelte')]
readonly class InertiaSvelteInitCommand implements CommandInterface
{
    public function __construct(
        private PackageJsonUpdater $packageJson,
        private InertiaSveltePublisherInterface $publisher,
        private InertiaSvelteViteConfigUpdater $viteConfigUpdater,
    ) {}
    ...
    public function execute(
        ...
        $configResult = $this->viteConfigUpdater->ensureSvelteConfig($force, $dryRun);
        $entrypointResult = $this->publisher->publishJsEntrypoint($force, $dryRun);

        $this->report($result, $dryRun, $output);
        $this->reportConfig($configResult, 'Svelte-aware Vite config', $output);
        $this->reportConfig($entrypointResult, 'Inertia Svelte entrypoint', $output);
